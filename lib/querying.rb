def select_books_titles_and_years_in_first_series_order_by_year
  "SELECT books.title, books.year FROM books GROUP BY books.year HAVING books.series_id = 1;"
end

def select_name_and_motto_of_char_with_longest_motto
  "SELECT characters.name, characters.motto AS longest FROM characters ORDER BY longest ASC LIMIT 1;"
end
...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(character_books.book_id) AS test FROM characters INNER JOIN character_books ON characters.id = character_books.character_id GROUP BY characters.name ORDER BY test DESC;"
end
